...
function readFact()
	...
	for index, line in pairs(fact_file) do
		...
		elseif key == "event" then
			table.insert(event, value)
		elseif key == "birth" then
			table.insert(birth, value)
		end
	end
end
...
function conky_main(  )
	...
	start_y = conky_window.height/3.5;
	...
	if status == 'FILLED' then
	  ...
	  _, y, _ = multiText("今日の出来事", start_x + box_width*(0.1) , y + box_height*(0), box_width*(0.90), box_height, box_height*(0.14), 'Text Me One', extents, font_ext, options);
	  fact =  event[math.random(#event)]
	  _, y, _ = multiText(fact, start_x + box_width*(0.1) , y + box_height*(0), box_width*(0.90), box_height, box_height*(0.14), 'Text Me One', extents, font_ext, options);
	  _, y, _ = multiText("誕生日", start_x + box_width*(0.1) , y + box_height*(0.1), box_width*(0.90), box_height, box_height*(0.14), 'Text Me One', extents, font_ext, options);
	  fact =  birth[math.random(#birth)]
	  _, y, _ = multiText(fact, start_x + box_width*(0.1) , y + box_height*(0), box_width*(0.90), box_height, box_height*(0.14), 'Text Me One', extents, font_ext, options);
	end

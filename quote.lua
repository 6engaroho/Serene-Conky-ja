...
-- reads the quote from Downloads/quote.cml
function readQuote()
    -- read the quote file
    ...
    q_number = math.floor(q_number /2)
end
...
function conky_main(  )
    ...
    local start_x = conky_window.width/50
    local  start_y = 0
    local x = start_x
    local y  = start_y

    -- lets print the quotes
    start_y = conky_window.height/3.8;
    ...
    if quote['status'] == 'FILLED' then
      ...
      local which_quote = math.random(q_number) 
      -- print the quote
      options.halign = 0
